...
import type { MembreProjet, TacheComplete } from '@/interfaces';

interface EditTaskModalProps {
  isOpen: boolean;
  onClose: () => void;
  onSubmit: (taskId: number, taskData: {
    titre: string;
    description: string;
  ...
  useEffect(() => {
    if (task && isOpen) {
      setFormData({
        ...
        date_limite: task.dateLimite || '',
        assignedUsers: [] // TODO: Récupérer les membres assignés depuis l'API
      });
      setErrors({});
    }
  }, [task, isOpen]);
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div className="flex justify-end gap-3 mt-6">
            ...
            <button
              type="submit"
              disabled={isLoading}
              className="px-4 py-2 bg-tf-saffron text-white rounded-lg hover:bg-yellow-600 disabled:opacity-50 transition-colors flex items-center gap-2"
            >
              {isLoading ? (
                <>
                  <LoaderDots size="sm" />
